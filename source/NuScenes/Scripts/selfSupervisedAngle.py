...
class TrainDataset(Dataset):
    def __init__(self, angleModel, device, dictionary, percentage, root, transform):
        ...
        printing = args.labelNumber in [2, 3, 4] and args.trainingCycles == 1
    ...
    def getDetections(self, dictionary, percentage):
        ...
        labelDir = '%s/training/label_0%d' % (self.root, args.labelNumber)
        ...
        for sceneIndex, objectLabels in sceneLabelsDict.items():
            ...
            self.mainIndicesDict[sceneIndex] = {}
            ...
            for labelNumber, objectLabel in enumerate(objectLabels[:endIndex]):
                if objectLabel['type'] == 'car':
                    ...
                    stationaryCondition = not objectLabel['moving']

                    if sizeCondition and stationaryCondition:
                        self.boundingBoxes.append((left, top, right, bottom))
                        mainIndex = len(self.toFrameIndexList)

                        if objectId in self.mainIndicesDict[sceneIndex]:
                            self.mainIndicesDict[sceneIndex][objectId].append(mainIndex)
                        else:
                            self.mainIndicesDict[sceneIndex][objectId] = [mainIndex]
                    ...
                    elif not stationaryCondition:
                        if objectId in self.mainIndicesDict[sceneIndex]:
                            originalId = objectId
                            objectId += 1000 

                            while objectId in self.mainIndicesDict[sceneIndex]:
                                objectId += 1000

                            dictionary = self.mainIndicesDict[sceneIndex]
                            dictionary[objectId] = dictionary.pop(originalId)

    def getOutputAngle(self, angleModel, device, mainIndex):
        frameIndex = self.toFrameIndexList[mainIndex]
        sceneIndex = self.toSceneIndexList[mainIndex]
    ...
    def getSelfSupervisedTargets(self, angleModel, device, printing=False):
        ...
        for sceneIndex in self.mainIndicesDict.keys():
            for objectId in self.mainIndicesDict[sceneIndex].keys():
                objectMainIndices = self.mainIndicesDict[sceneIndex][objectId]
